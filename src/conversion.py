...
for k, v in belgian_keyboard.items():
    if isinstance(v[0], tuple):  # Pour les caractères nécessitant plusieurs séquences de touches
        converted_value = []
        for tup in v:
            converted_tuple = ''.join(f'chr({num})' for num in tup)
            converted_value.append(converted_tuple)
        dict_converted[k] = converted_value
    else:
        dict_converted[k] = ''.join(f'chr({num})' for num in v)

# Affichage du dictionnaire converti pour vérification
